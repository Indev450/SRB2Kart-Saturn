...
ps_metric_t ps_hw_numpolys = {};
ps_metric_t ps_hw_numverts = {};
ps_metric_t ps_hw_numcalls = {};
ps_metric_t ps_hw_numshaders = {};
ps_metric_t ps_hw_numtextures = {};
ps_metric_t ps_hw_numpolyflags = {};
...
void HWR_Lighting(FSurfaceInfo *Surface, INT32 light_level, extracolormap_t *colormap, const boolean directional)
{
	...
	if (!HWR_UseShader())
	{
		...
		fade_alpha = CLAMP(fade_alpha, 0.0f, 1.0f);
...
static void HWR_SkyWallList_Add(FOutVector *wallVerts)
{
	...
	else if (skyWallVertexArraySize >= skyWallVertexArrayAllocSize)
	{
		...
		skyWallVertexArray = static_cast<FOutVector*>(Z_Realloc(skyWallVertexArray, sizeof(FOutVector) * 4 * skyWallVertexArrayAllocSize, PU_STATIC, NULL));
	}
...
#define SLOPEPARAMS(slope, end1, end2, normalheight) \
	...
	if (gl_backsector)
	{
		...
		if ((gl_midtexture && HWR_BlendMidtextureSurface(&Surf)) || gl_portal_state == GLPORTAL_STENCIL || gl_portal_state == GLPORTAL_DEPTH || gl_drawing_stencil)
		{
			...
			fixed_t texheight = textureheight[gl_midtexture];
			...
			else
            {
				...
				popentopslope    = std::min(worldtopslope, worldhighslope);
				popenbottomslope = std::max(worldbottomslope, worldlowslope);
			}

			// Find the wall's coordinates
			const fixed_t midtexheight = texheight * repeats;

			fixed_t rowoffset = gl_sidedef->rowoffset;
			...
			if (gl_linedef->flags & ML_EFFECT2)
			{
				...
				{
					polybottom = std::max(gl_frontsector->floorheight, gl_backsector->floorheight) + rowoffset;
					polytop = polybottom + midtexheight;
				}
				// Peg it to the ceiling
				else
				{
					polytop = std::min(gl_frontsector->ceilingheight, gl_backsector->ceilingheight) + rowoffset;
			...
			hS = std::min(highcutslope, polytopslope);
			lS = std::max(polybottomslope, lowcutslope);
	...
	if (!gl_drawing_stencil && gl_backsector && gl_frontsector->tag != gl_backsector->tag && (gl_backsector->ffloors || gl_frontsector->ffloors))
	{
		...
		auto process_ffloors = [&](boolean backsec)
		{
			...
			// i suck at naming stuff lul
			sector_t* main_sector = backsec ? gl_backsector : gl_frontsector;
			sector_t* other_sector = backsec ? gl_frontsector : gl_backsector;

			for (rover = main_sector->ffloors; rover; rover = rover->next)
			{
				const ffloortype_e roverflags = rover->flags;

				if (!(roverflags & FF_EXISTS) || !(roverflags & FF_RENDERSIDES))
					continue;

				if (backsec && (roverflags & FF_INVERTSIDES))
					continue;

				if (!backsec && !(roverflags & FF_ALLSIDES))
					continue;
				...
				const side_t *side = R_GetFFloorSide(gl_curline->linedef, rover, main_sector);
				...
				{
					const line_t *newline = R_GetFFloorLine(gl_curline->linedef, rover, main_sector);
					do_texture_skew = newline->flags & ML_DONTPEGTOP;
					dont_peg_bottom = newline->flags & ML_DONTPEGBOTTOM;
				}
				else
				{
					do_texture_skew = rover->master->flags & ML_DONTPEGTOP;
				...
				else
				{
					...
					fixed_t texturevpeg = side->rowoffset;
				...
				else
				{
					...
					if (other_sector->numlights)
						HWR_SplitWall(other_sector, wallVerts, texnum, noencore, &Surf, roverflags, rover, blendmode);
					else
					{
...
namespace
{
	enum class AddLineType
	{
		kPortal,
		kNormal,
	};

template <AddLineType Type>
static inline void DoAddLine(seg_t* line, angle_t angle1, angle_t angle2)
{
...
static void HWR_AddLine(seg_t *line)
{
	...
	if (UNLIKELY(HWR_UsePortals()))
		DoAddLine<AddLineType::kPortal>(line, angle1, angle2);
	else
		DoAddLine<AddLineType::kNormal>(line, angle1, angle2);
}
};
		...
		for (rover = gl_frontsector->ffloors; rover; rover = rover->next)
		{
			...
			auto render_plane = [&](boolean bottom)
			{
				...
				else
				{
					...
					HWR_RenderPlane(sub, &extrasubsectors[num], !bottom, roverheight, HWR_RippleBlend(gl_frontsector, rover, false)|PF_Occlude, *gl_frontsector->lightlist[light].lightlevel, flat->lumpnum,
...
static void HWR_RenderBSPNode(INT32 bspnum)
{
	...
	while (!(bspnum & NF_SUBSECTOR))  // Keep going until found a subsector
	{
		...
		if constexpr (Type == BspType::kPortal)
		{
			// Recursively divide front space (toward the viewer).
			if (HWR_PortalCheckBBox(bsp->bbox[side]))
				HWR_RenderBSPNode<BspType::kPortal>(bsp->children[side]);
...
static inline void HWR_ApplyDispoffset(gl_vissprite_t *spr, FOutVector *wallVerts, const boolean papersprite)
{
	...
	const float sprdist = sqrtf((spr->x1 - gl_viewx)*(spr->x1 - gl_viewx) + (spr->z1 - gl_viewy)*(spr->z1 - gl_viewy) + (spr->gzt - gl_viewz)*(spr->gzt - gl_viewz));
	const float distfact = ((2.0f*spr->dispoffset) + 20.0f) / sprdist;
...
static void HWR_AddTransparentPolyobjectFloor(lumpnum_t lumpnum, polyobj_t *polysector, boolean isceiling, fixed_t fixedheight, INT32 lightlevel, INT32 alpha, sector_t *FOFSector, FBITFIELD blend, extracolormap_t *planecolormap)
{
	polyplaneinfo_t *polyplaneinfo = static_cast<polyplaneinfo_t*>(HWR_CreateDrawNode(DRAWNODE_POLYOBJECT_PLANE));
...
void HWR_BuildSkyDome(void)
{
	...
	// create vertex array
	if (!sky->data)
		sky->data  = static_cast<gl_skyvertex_t*>(malloc(sky->vertex_count * sizeof(sky->data[0])));
...
namespace
{
	enum class RenderViewpointType
	{
		kPortal,
		kNormal,
	};
...
static void HWR_RenderViewpoint(gl_portal_t *rootportal, int stencil_level, boolean allow_portals)
{
	...
	{
		if (allow_portals && stencil_level < cv_maxportals.value) // if recursion limit is not reached
		{
			gl_portal_t *portal;
			...
			HWR_RenderBSPNode<BspType::kPortal>((INT32)numnodes-1);
	...
	if constexpr (Type == RenderViewpointType::kPortal)
	{
		...
		if (allow_portals && portalclipline)
			HWR_RenderBSPNode<BspType::kPortal>((INT32)numnodes-1);
		else
