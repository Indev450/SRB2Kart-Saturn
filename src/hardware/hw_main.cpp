		...
		for (rover = gl_frontsector->ffloors; rover; rover = rover->next)
		{
			     ...
			     (viewz < topCullHeight && (rover->flags & FF_BOTHPLANES || rover->flags & FF_INVERTPLANES))))
			{
				...
				else
				{
					...
					HWR_RenderPlane(sub, &poly_subsectors[num], true, *rover->topheight, HWR_RippleBlend(gl_frontsector, rover, *rover->topheight < rover->target->ceilingheight-(20*FRACUNIT))|PF_Occlude, *gl_frontsector->lightlist[light].lightlevel, levelflats[*rover->toppic].lumpnum,
									rover->master->frontsector, 255, gl_frontsector->lightlist[light].extra_colormap);
				}
			}
		}
