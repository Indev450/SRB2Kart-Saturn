...
static UINT8 *MakeBlock(GLMipmap_t *glMipmap)
{
	...
	block = (UINT8 *)(Z_Malloc(blocksize*bpp, PU_HWRCACHE, &(glMipmap->data)));
...
static void HWR_GenerateTexture(INT32 texnum, GLMapTexture_t *gltex, boolean noencore)
{
	...
	gltex->mipmap.format = (GLTextureFormat_t)(textureformat);
	...
	gltex->mipmap.colormap = (GLColormap_t *)(Z_Calloc(sizeof(*gltex->mipmap.colormap), PU_HWRPATCHCOLMIPMAP, NULL));
	...
	{
		pdata = (UINT8 *)(W_CacheLumpNumPwad(patch->wad, patch->lump, PU_LEVEL));
...
void HWR_MakePatch(const patch_t *patch, GLPatch_t *glPatch, GLMipmap_t *glMipmap, boolean makebitmap)
{
	if (glMipmap->width == 0)
	{
		...
		glMipmap->format = (GLTextureFormat_t)(patchformat);
...
void HWR_FreeTexture(patch_t *patch)
{
	...
	if (patch->hardware)
	{
		GLPatch_t *glPatch = (GLPatch_t *)(patch->hardware);
...
static void HWR_PrecacheLevelTextures(void)
{
	...
	texturepresent = (char *)(calloc(gl_numtextures, sizeof(*texturepresent)));
...
void HWR_LoadMapTextures(size_t pnumtextures)
{
	...
	gl_textures = (GLMapTextureSet_t *)(calloc(gl_numtextures, sizeof(*gl_textures))); // *2 - 1 for encore-remapped texture and another for noencore texture (unused when not in encore), see: GLMapTextureSet_t
...
void HWR_GetFlat(lumpnum_t flatlumpnum, boolean noencoremap)
{
	...
	if (!glMipmap->colormap)
	{
		...
		if (!noencoremap && encoremap)
			colormap += COLORMAP_REMAPOFFSET;
#endif
		glMipmap->colormap = (GLColormap_t *)(Z_Calloc(sizeof(*glMipmap->colormap), PU_HWRPATCHCOLMIPMAP, NULL));
...
patch_t *HWR_GetCachedGLPatchPwad(UINT16 wadnum, UINT16 lumpnum)
{
	...
	if (!lumpcache[lumpnum])
	{
		...
		Patch_AllocateHardwarePatch((patch_t *)(ptr));
	}
...
static UINT32 HWR_CreateLightTable(UINT8 *lighttable)
{
	...
	RGBA_t *hw_lighttable = (RGBA_t *)(Z_Malloc(256 * 32 * sizeof(RGBA_t), PU_STATIC, NULL));
