...
#define GLSL_DOOM_COLORMAP_DITHER \
	...
	"}\n" \
	"float zFactor = clamp((z - 768.0) / (3072.0 - 768.0), 0.0, 1.0);\n" \
	"halfResolutionx = mix(halfResolutionx, halfResolutionx * 4.0, zFactor);\n" \
	"halfResolutiony = mix(halfResolutiony, halfResolutiony * 4.0, zFactor);\n" \
	...
	"float threshold = bayerMatrix[y*4 + x] / 11.0;\n" \
	"#else\n" \
