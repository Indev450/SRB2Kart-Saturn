...
boolean P_Move(mobj_t *actor, fixed_t speed)
{
	...
	{
		if (!P_MobjWasRemovedCompat(actor) && actor->flags & MF_FLOAT && floatok)
		{
			// must adjust height
			if (actor->z < tmfloorz)
...
void P_NewChaseDir(mobj_t *actor)
{
	...
	if (P_MobjWasRemovedCompat(actor) ||
		P_MobjWasRemovedCompat(actor->target))
		return;
	...
	if (d[1] != DI_NODIR && d[2] != DI_NODIR)
	{
		...
		if ((newdir != turnaround) && (P_TryWalk(actor) || P_MobjWasRemovedCompat(actor)))
			return;
	}
	...
	if (d[1] != DI_NODIR)
	{
		...
		if (P_TryWalk(actor) || P_MobjWasRemovedCompat(actor))
...
void A_LobShot(void *thing)
{
	...
	if (P_MobjWasRemovedCompat(shot))
		return;
	...
	if (!P_MobjWasRemovedCompat(hitspot))
	{
...
void A_Boss1Laser(void *thing)
{
	...
	if (!(actor->flags2 & MF2_FIRING))
	{
		...
		if (!(actor->spawnpoint && actor->spawnpoint->options & MTF_AMBUSH))
		{
			...
			if (!P_MobjWasRemovedCompat(point))
			{
...
void A_1upThinker(void *thing)
{
	...
	if (!actor->tracer)
	{
		...
		if (P_MobjWasRemovedCompat(actor->tracer))
			return;
...
void A_MonitorPop(void *thing)
{
	...
	if (!P_MobjWasRemovedCompat(remains))
	{
		if (actor->eflags & MFE_VERTICALFLIP)
		{
	...
	if (item != 0)
	{
		...
		if (!P_MobjWasRemovedCompat(newmobj))
		{
...
static void P_DoBossDefaultDeath(mobj_t *mo)
{
	...
	for (th = thinkercap.next; th != &thinkercap; th = th->next)
	{
		...
		if (!mo2 || mo2->type != MT_BOSSFLYPOINT)
			continue;
...
static void P_SpawnBossJunk(mobj_t *mo)
{
	mobj_t *mo2;
...
void A_AttractChase(void *thing)
{
		...
		&& actor->info->reactiontime && actor->type != (mobjtype_t)actor->info->reactiontime)
	{
		...
		if (!P_MobjWasRemovedCompat(newring))
		{
...
void A_DropMine(void *thing)
{
	...
	if (!P_MobjWasRemovedCompat(mine))
	{
		if (actor->eflags & MFE_VERTICALFLIP)
...
void A_ThrownRing(void *thing)
{
	...
	for (; ; actor->lastlook = (actor->lastlook + 1) & PLAYERSMASK)
	{
		...
		if (!player->mo || (player->mo->health <= 0)) // dead
			continue;
...
void A_JetbThink(void *thing)
{
	...
	if (actor->target)
	{
			...
			&& (actor->target->z >= actor->floorz))
		{
			...
			if (!P_MobjWasRemovedCompat(bomb))
			{
...
void A_SlingAppear(void *thing)
{
	...
	while (mlength > 0)
	{
		...
		if (!P_MobjWasRemovedCompat(spawnee))
		{
...
void A_Boss2Chase(void *thing)
{
	...
	else
	{
		...
		if (leveltime % (speedvar*15/10)-1 == 0)
		{
			...
			if (!P_MobjWasRemovedCompat(goop))
			{
...
void A_Boss2PogoTarget(void *thing)
{
	...
	if (actor->info->missilestate) // spawn the pogo stick collision box
	{
		...
		if (!P_MobjWasRemovedCompat(pogo))
...
void A_Boss1Spikeballs(void *thing)
{
	...
	if (P_MobjWasRemovedCompat(ball))
		return;
...
void A_Boss3Path(void *thing)
{
	...
	if (actor->movecount & 2) // We've reached a firing point?
	{
		...
		if (actor->tracer && actor->tracer->state == &states[actor->tracer->info->missilestate])
			P_SetMobjState(actor, actor->info->missilestate);
		...
		case 0: // Boss1 jet fumes
			{
				...
				if (!P_MobjWasRemovedCompat(filler))
				{
...
void A_JawzExplode(void *thing)
{
	...
	if (!P_MobjWasRemovedCompat(truc))
	{
	...
	if (actor->extravalue1 == 1) // MODE: TARGETING
	{
		if (actor->tracer && actor->tracer->health && !(actor->tracer->player && actor->tracer->player->spectator))
		{
				...
				&& xyspeed > K_GetKartSpeed(actor->tracer->player, false)/4) // Don't display speedup lines at pitifully low speeds
			{
				...
				if (!P_MobjWasRemovedCompat(fast))
				{
...
void A_LightningFollowPlayer(void *thing)
{
	mobj_t *actor = thing;
...
void A_FZBoomSmoke(void *thing)
{
	...
	for (i = 0; i < 8+(4*var1); i++)
	{
		...
		if (P_MobjWasRemovedCompat(smoke))
			continue;
...
void A_RandomShadowFrame(void *thing)
{
	mobj_t *actor = thing;
	...
	if (!actor->extravalue1)	// Hack that spawns thoks that look like random shadows. Otherwise the state would overwrite our frame and that's a pain.
	{
		...
		if (!P_MobjWasRemovedCompat(fake))
		{
	...
	if (P_LookForPlayers(actor, false, false, 380<<FRACBITS))	// got target
	{
		...
		&& actor->z == actor->target->z)
		{
			...
			if (!P_MobjWasRemovedCompat(fire))
			{
...
void A_RoamingShadowThinker(void *thing)
{
	mobj_t *actor = thing;
	...
	if (!actor->extravalue2)
	{
		...
		if (P_LookForPlayers(actor, false, false, 256<<FRACBITS))	// got target
		{
			...
			&& !actor->target->player->kartstuff[k_spinouttimer])
			{
				...
				if (!P_MobjWasRemovedCompat(wind))
				{
...
void A_MayonakaArrow(void *thing)
{
	mobj_t *actor = thing;
...
void A_MementosTPParticles(void *thing)
{
	mobj_t *actor = thing;
	...
	for (; i < 4; i++)
	{
		...
		if (P_MobjWasRemovedCompat(particle))
			continue;
...
void A_ReaperThinker(void *thing)
{
	mobj_t *actor = thing;
...
void A_FlameParticle(void *thing)
{
	mobj_t *actor = thing;
...
void A_TrapShot(void *thing)
{
	...
	if (P_MobjWasRemovedCompat(missile))
		return;
...
void A_VileTarget(void *thing)
{
	...
	else
	{
		...
		for (i = 0; i < MAXPLAYERS; i++)
		{
			...
			fog = P_SpawnMobj(players[i].mo->x, players[i].mo->y,
...
void A_SpawnFreshCopy(void *thing)
{
	...
	if (P_MobjWasRemovedCompat(newObject))
		return;
