...
static void mobjnum_ht_linkedList_Wipe(void);
...
void* hashtable_Get(hashtable* table, const UINT32* key)
{
    ...
    UINT64 hash = hash_key((const char*)key);
    size_t index = (size_t)(hash & (UINT64)(table->capacity - 1));

    // Loop till we find an empty entry.
    while (table->entries[index].key != NULL)
    {
        if (strcmp((const char*)key, table->entries[index].key) == 0)
        {
...
const char* hashtable_Set(hashtable* table, const UINT32* key, void* value)
{
    ...
    return ht_set_entry(table->entries, table->capacity, (const char*)key, value,
                        &table->length);
}
