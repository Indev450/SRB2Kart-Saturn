...
static boolean P_CrossSubsector2(size_t num, register los_t *los)
{
	seg_t *seg;
	INT32 count;
...
static boolean P_CrossBSPNode2(INT32 bspnum, register los_t *los)
{
	while (!(bspnum & NF_SUBSECTOR))
	{
		register node_t *bsp = nodes + bspnum;
		...
		{
			if (!P_CrossBSPNode2(bsp->children[side], los))
				return 0;  // cross the starting side
			else
				bspnum = bsp->children[side^1];  // cross the ending side
		}
	...
	return
		P_CrossSubsector2((bspnum == -1 ? 0 : bspnum & ~NF_SUBSECTOR), los);
}
...
boolean P_CheckSight2(mobj_t *t1, mobj_t *t2)
{
	...
	// the head node is the last node output
	return P_CrossBSPNode2((INT32)numnodes - 1, &los);
}